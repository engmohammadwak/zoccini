    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.orders.update", [$order->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('restaurants') ? 'is-invalid' : '' }}" name="restaurants_id" id="restaurants_id" required>
                    @foreach($restaurants as $id => $restaurants)
                        <option value="{{ $id }}" {{ (old('restaurants_id') ? old('restaurants_id') : $order->restaurants->id ?? '') == $id ? 'selected' : '' }}>{{ $restaurants }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type_id" id="type_id" required>
                    @foreach($types as $id => $type)
                        <option value="{{ $id }}" {{ (old('type_id') ? old('type_id') : $order->type->id ?? '') == $id ? 'selected' : '' }}>{{ $type }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('sitting_area') ? 'is-invalid' : '' }}" name="sitting_area_id" id="sitting_area_id">
                    @foreach($sitting_areas as $id => $sitting_area)
                        <option value="{{ $id }}" {{ (old('sitting_area_id') ? old('sitting_area_id') : $order->sitting_area->id ?? '') == $id ? 'selected' : '' }}>{{ $sitting_area }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('number_people') ? 'is-invalid' : '' }}" type="text" name="number_people" id="number_people" value="{{ old('number_people', $order->number_people) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('schedule_request') ? 'is-invalid' : '' }}" name="schedule_request" id="schedule_request">
                    ...
                    @foreach(App\Models\Order::SCHEDULE_REQUEST_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('schedule_request', $order->schedule_request) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('schedule_date') ? 'is-invalid' : '' }}" type="text" name="schedule_date" id="schedule_date" value="{{ old('schedule_date', $order->schedule_date) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('car_number_yes') ? 'is-invalid' : '' }}" name="car_number_yes" id="car_number_yes">
                    ...
                    @foreach(App\Models\Order::CAR_NUMBER_YES_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('car_number_yes', $order->car_number_yes) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('car_number') ? 'is-invalid' : '' }}" type="text" name="car_number" id="car_number" value="{{ old('car_number', $order->car_number) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('delivery') ? 'is-invalid' : '' }}" name="delivery" id="delivery">
                    ...
                    @foreach(App\Models\Order::DELIVERY_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('delivery', $order->delivery) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('delivery_company') ? 'is-invalid' : '' }}" name="delivery_company_id" id="delivery_company_id">
                    @foreach($delivery_companies as $id => $delivery_company)
                        <option value="{{ $id }}" {{ (old('delivery_company_id') ? old('delivery_company_id') : $order->delivery_company->id ?? '') == $id ? 'selected' : '' }}>{{ $delivery_company }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('cansel_reason') ? 'is-invalid' : '' }}" name="cansel_reason_id" id="cansel_reason_id">
                    @foreach($cansel_reasons as $id => $cansel_reason)
                        <option value="{{ $id }}" {{ (old('cansel_reason_id') ? old('cansel_reason_id') : $order->cansel_reason->id ?? '') == $id ? 'selected' : '' }}>{{ $cansel_reason }}</option>
                    @endforeach
                </select>
