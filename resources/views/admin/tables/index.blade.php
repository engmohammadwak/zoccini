...
<div style="padding:24px;">
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:22px;flex-wrap:wrap;gap:12px;">
        <div style="display:flex;align-items:center;gap:12px;">
            <div style="width:44px;height:44px;border-radius:12px;background:linear-gradient(135deg,#6366f1,#818cf8);display:flex;align-items:center;justify-content:center;color:#fff;font-size:19px;box-shadow:0 4px 14px rgba(99,102,241,.3);">
                <i class="fas fa-table"></i>
            </div>
            <div>
                <h1 style="font-size:1.2rem;font-weight:800;color:var(--z-text);margin:0;line-height:1.2;">Tables</h1>
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(175px,1fr));gap:14px;margin-bottom:24px;">
        <div style="background:var(--z-card-bg);border:1px solid var(--z-card-border);border-radius:14px;padding:18px;box-shadow:var(--z-card-shadow);display:flex;align-items:center;gap:13px;">
            <div style="width:44px;height:44px;border-radius:12px;background:linear-gradient(135deg,#6366f1,#818cf8);display:flex;align-items:center;justify-content:center;color:#fff;font-size:18px;flex-shrink:0;box-shadow:0 4px 12px rgba(99,102,241,.3);"><i class="fas fa-table"></i></div>
            <div>
                <div style="font-size:1.55rem;font-weight:800;color:var(--z-text);line-height:1;">{{ $total }}</div>
                <div style="font-size:0.72rem;color:var(--z-text-faint);margin-top:3px;font-weight:600;">Tables</div>
            </div>
        </div>
    </div>
    ...
    <div style="background:var(--z-card-bg);border:1px solid var(--z-card-border);border-radius:16px;box-shadow:var(--z-card-shadow);overflow:hidden;">
        <div style="display:flex;align-items:center;justify-content:space-between;padding:18px 22px;border-bottom:1px solid var(--z-border);background:var(--z-surface-2);">
            <div style="display:flex;align-items:center;gap:10px;">
                <div style="width:36px;height:36px;border-radius:10px;background:rgba(99,102,241,.1);display:flex;align-items:center;justify-content:center;color:#6366f1;font-size:15px;"><i class="fas fa-table"></i></div>
                <div>
                    <div style="font-size:0.9rem;font-weight:700;color:var(--z-text);">Tables</div>
                    <div style="font-size:0.72rem;color:var(--z-text-faint);">{{ $total }} {{ trans('global.entries') }}</div>
                </div>
            </div>
            @can('table_create')
            <a href="{{ route('admin.tables.create') }}" style="display:inline-flex;align-items:center;gap:7px;padding:8px 16px;background:#6366f1;color:#fff;border-radius:10px;font-size:0.8rem;font-weight:700;text-decoration:none;box-shadow:0 3px 10px rgba(99,102,241,.3);transition:background .18s;"
               onmouseover="this.style.background='#4f46e5'" onmouseout="this.style.background='#6366f1'">
                <i class="fas fa-plus" style="font-size:0.75rem;"></i>
                {{ trans('global.add') }} Table
            </a>
            @endcan
        </div>
        <div style="padding:16px 22px;overflow-x:auto;">
            <table class="table datatable-Table" style="width:100%;">
                <thead>
                    <tr>
                        <th width="10"></th>
                        <th style="font-size:0.72rem;font-weight:700;color:var(--z-text-muted);text-transform:uppercase;letter-spacing:.06em;">Number</th>
                        <th style="font-size:0.72rem;font-weight:700;color:var(--z-text-muted);text-transform:uppercase;letter-spacing:.06em;">Capacity</th>
                        <th style="font-size:0.72rem;font-weight:700;color:var(--z-text-muted);text-transform:uppercase;letter-spacing:.06em;">Area</th>
                ...
                <tr data-entry-id="{{ $table->id }}">
                    <td></td>
                    <td>
                        <span style="background:#eef2ff;color:#4338ca;padding:3px 10px;border-radius:8px;font-weight:700;font-size:0.83rem;">
                            #{{ $table->number ?? $table->table_number ?? $table->id }}
                        </span>
                    </td>
                    <td style="font-size:0.83rem;color:var(--z-text-muted);">
                        {{ $table->capacity ?? '—' }}
                        @if($table->capacity ?? null)<span style="color:var(--z-text-faint);font-size:0.75rem;"> seats</span>@endif
                    </td>
                    <td style="font-size:0.82rem;color:var(--z-text-muted);">{{ optional($table->sittingArea)->name_en ?? '—' }}</td>
                    <td style="font-size:0.82rem;color:var(--z-text-muted);">{{ optional($table->restaurant)->name_en ?? '—' }}</td>
                    <td>
                        @php $s = optional($table->tableStatus)->name_en ?? ($table->status ?? null); @endphp
                        @if($s)
                        <span style="background:#f0fdf4;color:#166534;padding:3px 9px;border-radius:7px;font-size:0.78rem;font-weight:600;">{{ $s }}</span>
                        @else
                    ...
                    <td>
                        <div style="display:flex;gap:5px;">
                            @can('table_show')
                            <a href="{{ route('admin.tables.show',$table->id) }}" title="{{ trans('global.view') }}"
                            ...
                            @endcan
                            @can('table_edit')
                            <a href="{{ route('admin.tables.edit',$table->id) }}" title="{{ trans('global.edit') }}"
                            ...
                            @endcan
                            @can('table_delete')
                            <form action="{{ route('admin.tables.destroy',$table->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('{{ trans('global.areYouSure') }}')">@csrf @method('DELETE')
...
$(function(){
    ...
    $('.datatable-Table:not(.ajaxTable)').DataTable({
        buttons: dtButtons,
        order: [[1, 'asc']],
