            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type">
                    ...
                    @foreach(App\Models\Favorite::TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('type', $favorite->type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('item') ? 'is-invalid' : '' }}" name="item_id" id="item_id">
                    @foreach($items as $id => $item)
                        <option value="{{ $id }}" {{ (old('item_id') ? old('item_id') : $favorite->item->id ?? '') == $id ? 'selected' : '' }}>{{ $item }}</option>
                    @endforeach
                </select>
                @if($errors->has('item'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('restaurant') ? 'is-invalid' : '' }}" name="restaurant_id" id="restaurant_id">
                    @foreach($restaurants as $id => $restaurant)
                        <option value="{{ $id }}" {{ (old('restaurant_id') ? old('restaurant_id') : $favorite->restaurant->id ?? '') == $id ? 'selected' : '' }}>{{ $restaurant }}</option>
                    @endforeach
                </select>
