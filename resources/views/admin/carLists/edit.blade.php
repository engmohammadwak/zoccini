...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.carList.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.car-lists.update", [$carList->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('car_brand') ? 'is-invalid' : '' }}" name="car_brand_id" id="car_brand_id">
                    @foreach($car_brands as $id => $car_brand)
                        <option value="{{ $id }}" {{ (old('car_brand_id') ? old('car_brand_id') : $carList->car_brand->id ?? '') == $id ? 'selected' : '' }}>{{ $car_brand }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('car_type') ? 'is-invalid' : '' }}" name="car_type_id" id="car_type_id">
                    @foreach($car_types as $id => $car_type)
                        <option value="{{ $id }}" {{ (old('car_type_id') ? old('car_type_id') : $carList->car_type->id ?? '') == $id ? 'selected' : '' }}>{{ $car_type }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('car_color') ? 'is-invalid' : '' }}" name="car_color_id" id="car_color_id">
                    @foreach($car_colors as $id => $car_color)
                        <option value="{{ $id }}" {{ (old('car_color_id') ? old('car_color_id') : $carList->car_color->id ?? '') == $id ? 'selected' : '' }}>{{ $car_color }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('pate_number') ? 'is-invalid' : '' }}" type="text" name="pate_number" id="pate_number" value="{{ old('pate_number', $carList->pate_number) }}">
