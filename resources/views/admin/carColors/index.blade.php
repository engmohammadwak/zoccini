...
<div style="padding:24px;">
    ...
    <x-admin-table title="Car Colors" icon="fas fa-palette" color="pink" datatableClass="datatable-CarColor" :count="$carColors->count()" createPermission="car_color_create" :createRoute="route('admin.car-colors.create')" :createLabel="trans('global.add').' Color'">
        ...
        <x-slot name="tbody">
            ...
            <tr data-entry-id="{{ $color->id }}">
                ...
                <td>
                    @if($color->hex ?? $color->color ?? null)
                    <span style="display:inline-flex;align-items:center;gap:6px;">
                        <span style="width:18px;height:18px;border-radius:50%;background:{{ $color->hex ?? $color->color }};border:2px solid #e2e8f0;display:inline-block;"></span>
                        <span style="font-size:0.78rem;font-family:monospace;color:#475569;">{{ $color->hex ?? $color->color }}</span>
                    </span>
                    @endif
                </td>
...
@section('scripts')
@parent
<script>$(function(){ $.extend(true,$.fn.dataTable.defaults,{orderCellsTop:true,order:[[1,'asc']],pageLength:25}); $('.datatable-CarColor:not(.ajaxTable)').DataTable({buttons:$.extend(true,[],$.fn.dataTable.defaults.buttons)}); });</script>
@endsection
