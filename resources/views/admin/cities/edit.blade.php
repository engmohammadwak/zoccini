            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('countries') ? 'is-invalid' : '' }}" name="countries_id" id="countries_id">
                    @foreach($countries as $id => $countries)
                        <option value="{{ $id }}" {{ (old('countries_id') ? old('countries_id') : $city->countries->id ?? '') == $id ? 'selected' : '' }}>{{ $countries }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\City::STATUS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('status') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="status_{{ $key }}" name="status" value="{{ $key }}" {{ old('status', $city->status) === (string) $key ? 'checked' : '' }}>
